...
extern "C" {
    fn ghostty_vt_render_owned_begin(
        ...
        cursor_active_x: u16,
        cursor_active_y: u16,
        cursor_style: *const c_void,
        cursor_cell: *const c_void,
        viewport_pin: *const c_void,
...
pub unsafe extern "C" fn ghostty_rust_terminal_owned_render_state_update(
    ...
    unsafe {
        ...
        let result = ghostty_vt_render_owned_begin(
            ...
            cursor.x,
            cursor.y,
            ptr::from_ref(&cursor.style).cast(),
            cursor_cell_ref.cast(),
            ptr::from_ref(&viewport_pin).cast(),
            background,
            foreground,
            cursor_rgb.is_some(),
        ...
        while let Some(row_pin) = row_it.next() {
            ...
            let row_result = ghostty_vt_render_owned_row(
                ...
                alloc,
                y,
                ptr::from_ref(&row_pin).cast(),
                ptr::from_ref(row_ref).cast(),
                cells_ptr.as_ptr().cast(),
                cols,
                row_dirty,
                sel_present,
                sel_start,
